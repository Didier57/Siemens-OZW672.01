...
class Datapoint:
    ...
    The ``segments`` (the chain of topic titles inside the controller, for
    example ``["Diagnostic consommateurs", "Température extérieure"]``) are the
    stable identity of a datapoint: the numeric identifier is generated by the
    OZW672 for the plant it is wired to and changes when the plant or the
    server parameters are refreshed. ``path`` is the same chain joined with
    ``/`` for display only, because a title may itself contain a slash.
    """
    ...
    def key(self) -> str:
        ...
        The segments are joined with ``/``, which is unambiguous as a storage
        key even when a title contains a slash, because the segments are kept
        separately in ``segments`` and never recovered by splitting the key.
        """
        if self.segments:
