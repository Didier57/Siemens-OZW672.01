...
Version 2 removes the built-in datapoint catalog: datapoint identifiers are generated by the OZW672 for each plant, so a shipped list could never match every installation. Existing entries are migrated automatically — the custom datapoints declared with 1.x are kept as id based entries — but the datapoints of the old built-in catalog are no longer created. Open **Configure → Add datapoints** and select them again by topic.
